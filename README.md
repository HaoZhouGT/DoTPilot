![DoTPilot AI Agent](assets/dotpilot-banner.png)
...
- An OpenAI API key (for the cloud LLM backend)
...
│   ├── cloud_backend.py   # OpenAI Chat Completions API (vision + tool use)
...
| `AgentApiKey` | string | *(none)* | OpenAI API key for cloud backend |

The cloud backend currently uses `gpt-4o-mini` with an 8-second timeout. These can be adjusted in `sunnypilot/agentd/backends/cloud_backend.py`.
