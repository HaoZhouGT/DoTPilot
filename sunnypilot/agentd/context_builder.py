...
class ContextBuilder:
  ...
  def build(self, sm: messaging.SubMaster, frame_jpeg: bytes | None) -> dict:
    return {
      ...
      "network": self._build_network(sm),
  ...
  def _build_network(self, sm: messaging.SubMaster) -> dict:
    ds = sm['deviceState']
    net_info = ds.networkInfo

    result = {
      "network_type": str(ds.networkType),
      "network_strength": str(ds.networkStrength),
      "network_metered": bool(ds.networkMetered),
    }

    # Carrier/operator info (only populated for cellular)
    if net_info.operator:
      result["operator"] = str(net_info.operator)
    if net_info.technology:
      result["technology"] = str(net_info.technology)
    if net_info.state:
      result["network_state"] = str(net_info.state)

    # Athena ping time (monotonic nanoseconds; convert to seconds ago)
    last_ping_ns = ds.lastAthenaPingTime
    if last_ping_ns > 0:
      now_ns = int(time.monotonic() * 1e9)
      seconds_ago = (now_ns - last_ping_ns) / 1e9
      result["last_athena_ping_s_ago"] = round(max(0.0, seconds_ago), 1)
