...
class RoadInspectionOverlay(Widget):
  def __init__(self, avoid_left_hud: bool = False):
    super().__init__()
    self._avoid_left_hud = avoid_left_hud
  ...
  def _update_state(self) -> None:
    ...
    self._alpha_filter.update(1.0 if self._has_visible_issue() or self._fallback_advisory() else 0.0)
  ...
  def _fallback_advisory(self) -> str:
    return _clean_text(ui_state.llm_advisory, 72) if ui_state.params.get_bool("LLMAgentEnabled") else ""
  ...
  def _render(self, rect: rl.Rectangle) -> bool:
    ...
    x, y, width = self._layout(rect)
    if self._has_visible_issue():
      title = self._title_text()
      detail = _clean_text(self._record.get("description") if self._record else None, 120)
      meta = self._meta_text()
    else:
      title = self._fallback_advisory()
      detail = ""
      meta = "LLM Agent"
  ...
  def _layout(self, rect: rl.Rectangle) -> tuple[float, float, int]:
    x = rect.x + LEFT_MARGIN
    if self._avoid_left_hud and rect.width > 1100:
      x += STANDARD_HUD_LEFT_GUTTER
    y = rect.y + TOP_MARGIN
    width = min(MAX_WIDTH, max(0, int(rect.x + rect.width - x - LEFT_MARGIN)))
    return x, y, width
